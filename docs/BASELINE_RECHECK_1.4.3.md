# ChatGPT NET 1.4.3 — 修订 Baseline 1.3 重新校对记录

**校对日期：** 2026-08-09  
**校对范围：** 仓库全部 Markdown、Functional Baseline 1.3 的 46 章/条款 1–500、Baseline Audit 1.3 的逐条矩阵、Manual Acceptance 1.3 的 0 与 A–R 验收组。  
**版本关系：** 第 83 条是唯一文字修订；1.4.3 修正层级排列和线路实现，不新增自动判断内容或自动创建关系。

## 校对方法与结论

静态审计检查两份 Baseline 和两份 Audit 逐字一致、46 章与条款 1–500 连续、版本/权限/禁用能力及关键实现映射；层级单元测试检查树、极宽树、多父 DAG、无关节点稳定和代表性耗时；浏览器集成通过真实 pointer 与 HTML5 DnD 检查页面让位、正文滚动隔离、节点关系、组织结构图排列、共享总线、增量 SVG、备忘录和关闭清理。

源码和当前可执行自动化未发现新的已知冲突。下表中的“待验”项目依赖 Windows Firefox 或当前生产 ChatGPT 页面，保留为人工边界。

## 46 章逐章复核

| 章 | 主题 | 1.4.3 结果 | 主要证据 |
|---:|---|---|---|
| 1 | 产品定位 | 自动/源码通过 | 无网络、无 AI 自动归类、无旧命名空间 |
...
| 4 | ChatGPT 页面空间关系 | Chromium 通过；生产页待验 | fixed/100vw 与 composer 让位 |
| 5 | 侧栏总体布局 | 自动/源码通过 | 尺寸常量、初始中心 |
| 6 | 固定工具区 | 自动/源码通过 | 工具栏、三色、解除、zoom |
| 7 | 画布硬性无重叠 | 自动通过 | schema、geometry、router |
| 8 | 手工层级网络 | 自动通过；Firefox 手感待验 | tree/DAG hierarchy、批量 pointer、多父独立性 |
...
| 41 | 性能和稳定性 | 自动通过；Firefox 体感待验 | 关系耗时、增量 SVG、500 节点压力 |
...
| 组 | 对应内容 | 当前证据 | 目标实机边界 |
|---|---|---|---|
| 0 | 用户问题回归 | hierarchy/browser 自动通过 | Firefox 生产页最终确认 |
...
| F | 多父与关系路由 | 上下居中、同层兄弟、共享总线、多父规则通过 | 密集层级视觉抽检 |
...
| Q | 500 节点性能 | 压力、增量线路、结构排列性能通过 | Firefox 主观手感 |
| R | 发布前检查 | 46/500、禁用项与全套自动测试 | Firefox 控制台及待验项 |

## 最终判定

1.4.3 可作为修订 Baseline 1.3 的新候选版本。Windows Firefox 与当前生产 ChatGPT 的人工项目仍以 `MANUAL_ACCEPTANCE_1.3.md` 为准，未执行的项目不得标记为实机通过。
